...
namespace WebApi1C.Server.Services
{
    public class Service1C77
    {
        ...
        private string _objectCasheFile;
        ...
        private void ReadConfig()
        {
            ...
            _casheDir = configuration["CasheFolder"];
            if (_casheDir == null)
                throw new Exception(string.Format(errorGetConfigParamString, "CasheFolder"));
            _metadataFile = $"{_casheDir}/metadata1C7.json";
            _objectCasheFile = $"{_casheDir}/cashe1C7.json";
        }
        ...
        public async Task<object> GetDataFrom1C([NotNull] string selector, string key, bool fresh = false)
        {
            ...
            {
                string fileName = $"{_casheDir}/catalog_{metadata.Идентификатор}.json";
                ...
                if (!fresh)
                {
                    if (File.Exists(fileName))
                    {
                        ...
                        return System.Text.Json.JsonSerializer.Deserialize<List<Dictionary<string, object>>>(s);
                    }
                }
